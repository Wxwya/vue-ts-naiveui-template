...
const props = defineProps({
  ...
  row: {
    type: Object as PropType<SystemUser.UserRow>,
    default: () => ({})
  }
})

const departmentOptions = ref<GlobalOptions<number>[]>([])
const formData = ref<SystemUser.UserReq>({
  ...props.row,
  role_ids: props.row.roles?.map(r => r.id) ?? [],
  department_ids: props.row.departments?.map((d: any) => d.id) ?? [],
})
...
const rules = computed<FormRules>(() => {
  return formItemData.value.reduce((acc: FormRules, cur: FormItemRowStruct) => {
    if (cur.item.path === 'password' && props.row.id) return acc
    if (!cur.item.ruleType) return acc
    acc[cur.item.path!] = [{ required: true, trigger: [], message: cur.content?.placeholder as string, type: cur.item.ruleType }]
    return acc
  }, {})
})
