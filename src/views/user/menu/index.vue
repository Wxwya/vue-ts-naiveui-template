    ...
    },
    {
      ...
      render: (row) => (
        <div class="flex items-center gap-2">
          <NButton text type="info" onClick={() => onOpenModal("修改字典类型", row)}>修改</NButton>
          <NButton text type="info" onClick={() => push(`/user/sub-menu?id=${row.id}&prefix=${row.path}`)}>子菜单管理</NButton>
