    ...
    },
    {
      ...
      render: (row) => (
        <div class="flex items-center gap-2">
          <NButton text type="info" onClick={() => onOpenModal("修改权限", row)}>修改</NButton>
          <NButton text type="info" onClick={() => push(`/user/button-permissions?id=${row.id}&prefix=${row.permission_name}`)}>按钮权限管理</NButton>
