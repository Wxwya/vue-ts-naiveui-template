...
import type { DataTableColumns, DataTableRowKey } from "@/rely/page"
import { closeModal } from "@/rely/lib"
import UpModal from './upModal.vue'

const api = inject("api") as Api
const data = ref<SystemConfig.ConfigRow[]>([])
const loading = ref(false)
const rowIds = ref<DataTableRowKey[]>([])

const columns : DataTableColumns<SystemConfig.ConfigRow> = [
  ...
  { title: 'Value', key: 'value', className: "min-w-[180px]" },
  {
    ...
    render: (row) => <NSwitch value={row.enable} onUpdateValue={(val) => onUpdateEnable(row.id, val)} />
  },
  ...
  { title: '创建时间', key: 'create_time', className: "min-w-[220px]" },
  {
    ...
    render: (row) => (
      <div class="flex items-center gap-2">
        <NButton v-has="xwya:config:update" text type="info" onClick={() => onOpenModal('修改配置', row)}>修改</NButton>
        <NButton v-has="xwya:config:delete" text type="error" onClick={() => onDeleteTips(row)}>删除</NButton>
      </div>
    )
  }
]
...
const onOpenModal = (title: string, row?: SystemConfig.ConfigRow) => {
...
const onDeleteTips = (row: SystemConfig.ConfigRow) => {
  window.$dialog.warning({
    title: '温馨提示',
...
const onUpdateEnable = async (id: number, val: boolean) => {
